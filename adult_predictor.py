...
def predict(message, column_order, argument):
	...
	if argument == "keras":
		trow = np.vstack((trow.loc[0].values,trow.loc[0].values))
		return model.predict(trow)[0]
	else:
		return model.predict(trow)[0]


def start(model_id, messages_count, batch_id, argument):
	for msg in consumer:
		message = json.loads(msg.value)
		...
		if is_retraining_message(msg):
			...
			pred = predict(message['data'], column_order, argument)
...
if __name__ == '__main__':
	...
	start(model_id, messages_count, batch_id, args.hyper)
